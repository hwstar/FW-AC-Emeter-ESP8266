...
kvstoreGet(flash_handle_s *h, const char *key)
{
	...
	return (void *) h->sector->entries[idx].value;
}
...
ICACHE_FLASH_ATTR kvstore_open(unsigned cfg_sector_loc)
{
	...
	flash_handle_s *h = (flash_handle_s *) os_zalloc(sizeof(flash_handle_s));
...
/*
 * Return true if the flash sector needs to be written.
 */

bool ICACHE_FLASH_ATTR kvstore_dirty(flash_handle_s *h)
{
	return h->dirty;
}
 ...
 * Return the result as a binary blob of KVS_BLOB_SIZE. Returned
 * memory region must be freed after it is no longer needed.
 */ 
void * ICACHE_FLASH_ATTR kvstore_get_blob(flash_handle_s *h, const char *key)
{
	void *v = kvstoreGet(h, key);
	void *blob = (void *) os_zalloc(KVS_BLOB_SIZE);
	
	os_memcpy(blob, v, KVS_BLOB_SIZE);
...
/*
 * Write a blob to flash. Will return TRUE if successfull, otherwise
 * FALSE.
 */

bool ICACHE_FLASH_ATTR kvstore_put_blob(flash_handle_s *h, const char *key, const void *blob)
{
	unsigned idx = kvstoreIndex(h, key);	
	...
	/* Store the new value */
	os_memcpy(h->sector->entries[idx].value, blob, KVS_BLOB_SIZE);
...
{
	bool res;
	char *padded_value = (char *) os_zalloc(KVS_BLOB_SIZE);
	// Copy string to zero-initialzed buffer
	os_strncpy(padded_value, value, KVS_BLOB_SIZE);
	// Add a nul to end of buffer
	padded_value[KVS_BLOB_SIZE - 1] = 0;
	// Write the blob to flash
	res =  kvstore_put_blob(h, key, padded_value);
	// Free padded buffer
	os_free(padded_value);
	// Return result
	return res;
}
